...
def main():
    ...
    first_pir = MotionSensor(3)
	second_pir = MotionSensor(4)
    output = None
    if DEBUG:
    ...
    try:
        no_motion = True
        while True:
            first_pir.wait_for_motion()
			first_timestamp = int(datetime.now().strftime('%s')) * 1000
			second_pir.wait_for_motion()
			second_timestamp = int(datetime.now().strftime('%s')) * 1000
			#most likely wont differ, because a second is quite a long time
            if second_timestamp - first_timestamp > 1000:
			    payload = {'timestamp': first_timestamp, 'count': 1, 'type': type}
			else:
			    print('second_timestamp - first_timestamp')
			    payload = {'timestamp': first_timestamp, 'count': 1, 'type': type}
			r = requests.post(url, json=payload)
            second_pir.wait_for_no_motion()
            if DEBUG:
                writer.writerow([timestamp])
            else:
